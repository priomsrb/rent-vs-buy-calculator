...
describe("PrincipalPaid", () => {
  ...
  function calculateForYear(year: number) {
    return PrincipalPaid.calculateForYear({
      year,
      params,
      previousBreakdowns: [],
    });
  ...
  it("calculates principal paid correctly for a given year", () => {
    expect(calculateForYear(0)).toBeCloseTo(16702.4, 2);
    expect(calculateForYear(1)).toBeCloseTo(17210.42, 2);
    expect(calculateForYear(29)).toBeCloseTo(39823.89, 2);
  });
