...
describe('CouncilRatesPaid', () => {
  ...
  it('calculates council rates paid correctly for a given year', () => {
    ...
    let expectedCost = -params.councilRates;
    expect(cost).toBeCloseTo(expectedCost);
    expect(cost).toBeCloseTo(-1500);

    // Year 1
    cost = CouncilRatesPaid.calculateForYear({ params, year: 1 });
    expectedCost = -params.councilRates * (1 + params.propertyGrowth / 100);
    expect(cost).toBeCloseTo(expectedCost);
    ...
    expectedCost =
      -params.councilRates * Math.pow(1 + params.propertyGrowth / 100, 2);
    expect(cost).toBeCloseTo(expectedCost);
