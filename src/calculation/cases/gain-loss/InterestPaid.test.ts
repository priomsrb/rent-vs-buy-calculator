...
describe("InterestPaid", () => {
  ...
  function calculateForYear(year: number) {
    return InterestPaid.calculateForYear({
      params,
      year,
      previousBreakdowns: [],
    });
  ...
  it("calculates interest paid correctly for a given year", () => {
    expect(calculateForYear(0)).toBeCloseTo(-23771.58, 2);
    expect(calculateForYear(1)).toBeCloseTo(-23263.56, 2);
    expect(calculateForYear(29)).toBeCloseTo(-650.1, 2);
  });
